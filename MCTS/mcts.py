...
class MCTS:
    ...
    def run_mcts(self, state, network, temperature, deterministic):
        ...
        if (
            ...
            and self.root_exploration_eps > 0.0
        ):
            prior_prob = self.add_dirichlet_noise(
                prior_prob,
                eps=self.root_exploration_eps,
        ...
        for s in range(self.n_simulations):
            ...
            while node.is_expanded:
                node = node.best_child(
                    self, self.min_max_stats
                )  # pass MCTS object to have access to the config
            ...
            action = (
                torch.nn.functional.one_hot(action, num_classes=network.num_actions)
                .squeeze()
                .to(self.dev, dtype=torch.float32)
            )

            # Take a step in latent space
            h_state, rwd, pi_probs, value = network.recurrent_inference(
                h_state, action
